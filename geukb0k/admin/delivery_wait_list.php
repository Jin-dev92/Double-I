...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		...
		$chk_cnt = sql_fetch("select count(*) as cnt from nfor_cart where cart_id='$ct[cart_id]' and delivery_step='1'");
		sql_query("update nfor_cart set delivery_step='1' where ct_id='{$_POST['ct_id'][$k]}'");
		if(!$chk_cnt[cnt]){	// 배송준비중인게 없으면(첫번째 배송처리이면)
			$order = sql_fetch("select * from nfor_order where cart_id='$ct[cart_id]'");
			nfor_send("delivery_ready",$order[mb_email],$order[mb_hp],$order[mb_no],$order[od_id],$ct[ct_id]);
		}
	}

	alert("일괄배송준비중처리되었습니다","delivery_wait_list.php?$qstr");
	exit;
}


$sql_common = " from nfor_cart ";
$sql_search = " where pay_step='1' and delivery_step='0' and it_delivery='1' ";
...
<form name="flist" id="flist" method="post" action="delivery_wait_list.php">
...
<div class="btn_cen"><a href="javascript:nfor_list('배송준비','list_update')"><img src="img/delivery11_btn.gif" alt="배송준비처리"></a></div>
</form>
