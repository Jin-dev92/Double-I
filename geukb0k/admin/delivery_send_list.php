...
if($mode=="list_edit"){
	...
	alert("송장정보가 일괄 수정되었습니다","delivery_send_list.php?$qstr");
	exit;
}
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		...
		$ct = sql_fetch("select * from nfor_cart where ct_id='{$_POST['ct_id'][$k]}'");
		$order = sql_fetch("select * from nfor_order where cart_id='$ct[cart_id]'");
		$chk_cnt = sql_fetch("select count(*) as cnt from nfor_cart where cart_id='$ct[cart_id]' and delivery_step='3'");
		...
		if(!$chk_cnt[cnt] and $ct[it_delivery] and $ct[delivery_step]=="2"){
			$order = sql_fetch("select * from nfor_order where cart_id='$ct[cart_id]'");
			nfor_send("delivery_return_wait",$order[mb_email],$order[mb_hp],$order[mb_no],$order[od_id],$ct[ct_id]);
		}

		$chk_cart = sql_fetch("select count(*) as cnt from nfor_cart where cart_id='$ct[cart_id]' and pay_step='1'");
		if(!$chk_cart[cnt]){
	...
	alert("일괄 반품신청 처리되었습니다","delivery_send_list.php?$qstr");
	exit;
}


$sql_common = " from nfor_cart ";
$sql_search = " where pay_step='1' and delivery_step='2' and it_delivery='1' ";
...
<tr>
	<td align="left" height="50">주문수량 : <?=number_format($total_count)?>개</td>
	<td align="right">
	...
	<a href="delivery_send_list.php?mode=excel&<?=$qstr?>"><img src="img/ex_down.gif" alt="엑셀다운" align="absmiddle"></a>
	</form>
	</td>
</tr>
</table>
...
<form name="flist" id="flist" method="post" action="delivery_send_list.php">
...
<tr onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor=''" bgcolor="#FFFFFF">
	...
	<?
	if($row[delivery_company] and $row[delivery_step]=="2"){
		$wr_url = sql_fetch("select wr_url from nfor_delivery where wr_name='$row[delivery_company]'");
		echo "<br><a href='{$wr_url[wr_url]}{$row[delivery_number]}' target='_blank' class='btn_sml'><span>배송조회</span></a>";
	}
	?>
...
<div class="btn_cen">
<a href="javascript:nfor_list('반품신청','list_update')"><img src="img/delivery1111_btn.gif" alt="일괄반품신청"></a>

<a href="javascript:nfor_list('송장수정','list_edit')"><img src="img/delivery11112_btn.gif" alt="일괄송장수정"></a>
