...
$my_cancel_price_s = sql_fetch("select sum(my_cancel_price) as my_cancel_price_s from nfor_cart where pay_step='3' and cart_id='$order[cart_id]'");
$chk_my = $my_cancel_price_s[my_cancel_price_s]+$my_cancel_price;
if($chk_my > $order[money_price]) alert("적립금 사용액보다 취소요청금액이 더 큽니다");

$pg_cancel_price_s = sql_fetch("select sum(pg_cancel_price) as pg_cancel_price_s from nfor_cart where pay_step='3' and cart_id='$order[cart_id]'");
$chk_pg = $pg_cancel_price_s[pg_cancel_price_s]+$canc_amt;
if($chk_pg > $order[pg_price]) alert("PG결제금액보다 취소요청금액이 더 큽니다");
...
if($rStatus=="O"){
	...
	$pg_cancel_price = $canc_amt; // 취소요청된 금액
	$return_price = $my_cancel_price+$pg_cancel_price;

	sql_query("update nfor_cart set pay_step='3', od_canceldatetime=NOW(), pg_cancel_price='$pg_cancel_price', cancel_price='$return_price' where ct_id='$ct_id'");
	...
	$order = sql_fetch("select * from nfor_order where cart_id='$ct[cart_id]'");
	if($my_cancel_price){
		sql_query("update nfor_cart set my_cancel_price='$my_cancel_price' where ct_id='$ct[ct_id]'");
		insert_money($order[mb_no],$my_cancel_price,"적립금 상품구매 부분취소","8",$order[od_id],$ct[ct_id]);
	}


	$cancel_price = $order[cancel_price]+$return_price;	// 취소된금액 + 지금취소되는금액
	...
	$msg = iconv("euc-kr","utf-8",$rMessage1.$rMessage2.$rAuthNo);
	alert_close_refresh("주문취소에 실패하였습니다".$msg);
	exit;
}
