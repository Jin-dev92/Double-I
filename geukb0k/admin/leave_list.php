...
if($mode=="update"){
	...
	insert_money($mb[mb_no],$mb[mb_money]*-1,"회원탈퇴","11");

	//nfor_send("member_leave",$mb[mb_email],$mb[mb_hp],$mb[mb_no]);
	alert("선택한 인원에 대해서 탈퇴신청 처리되었습니다","leave_list.php?$qstr");
	exit;
}
...
if($mode=="list_update"){
	for($i=0; $i<count($chk); $i++){
		...
		$mb = sql_fetch("select * from nfor_member where mb_no='{$_POST['mb_no'][$k]}'");
		$sql = " update nfor_member
					set mb_id='',
						mb_name='',
						mb_password = '',
...
$sql_search = " where mb_level > 1 and mb_leavedatetime <> '' ";
...
if($mode=="excel"){
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue("Q1", "탈퇴사유")
				->setCellValue("R1", "탈퇴신청일자");
	...
	for($i=2; $row=sql_fetch_array($result); $i++){    
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("Q$i", "$row[mb_secession]")
					->setCellValue("R$i", "$row[mb_leavedatetime]");
	}
...
<tr>
	...
	<a href="leave_list.php?mode=excel&<?=$qstr?>"><img src="img/ex_down.gif" alt="엑셀다운" align="absmiddle"></a>
	</form>
	</td>
</tr>
</table>	

<form name="flist" id="flist" method="post" action="leave_list.php">
...
<tr onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor=''" bgcolor="#FFFFFF">
	...
	<td><?=substr($row[mb_leavedatetime],0,10)?></td>
	<td><a href="javascript:del('leave_list.php?mode=update&mb_no=<?=$row[mb_no]?>&<?=$qstr?>')"><img src="img/del_btn.gif"></a></td>
</tr>
<?
}
...
<div class="btn_cen"><a href="javascript:nfor_list('탈퇴신청','list_update')"><img src="img/sun_del.gif" alt="선택삭제"></a></div>
</form>
<?
include "tail.php";
?>