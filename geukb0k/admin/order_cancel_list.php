...
if($mode=="list_delete"){
	...
	for($i=0; $i<count($chk); $i++){
		...
		sql_query("delete from nfor_cart where cart_id='$order[cart_id]'");
	}
...
$sql_search = " where pay_step='3' or pay_step='5'";
...
if($mode=="excel"){
	...
	for($i=2; $row=sql_fetch_array($result); $i++){    
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("O$i", substr($row[od_canceldatetime],0,10));
	}
...
<tr>
	...
	<select name="date_type">
		...
		<option value="od_canceldatetime" <?=$date_type=="od_canceldatetime"?"selected":""?>>취소일자
	</select> 
...
<tr onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor=''" bgcolor="#FFFFFF">
	...
	<td><?=substr($row[od_canceldatetime],0,10)?></td>
</tr>
<?
}
...
<div class="btn_cen"><a href="javascript:nfor_list('주문삭제','list_delete')"><img src="img/sun11_del.gif" alt="주문삭제"></a></div>
