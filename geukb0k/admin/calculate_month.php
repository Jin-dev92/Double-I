...
for($i=1; $i<=$last_day; $i++){
	...
	$sql = "select count(*) as order_cnt, sum(it_total_cnt) as it_total_cnt, sum(delivery_price) as delivery_price, sum(cancel_price) as cancel_price, sum(total_price) as total_price, sum(money_price) as money_price, sum(card_price) as card_price, sum(iche_price) as iche_price, sum(banking_price) as banking_price from nfor_order where pay_step='1' and DATE_FORMAT(od_paydatetime,'%Y%m%d')='$ymd'";
	...
	$rows[$i][it_total_cnt] += $row[it_total_cnt];
...
<tr>
	...
	<a href="calculate_month.php?mode=excel&<?=$qstr?>"><img src="img/ex_down.gif" alt="엑셀다운" align="absmiddle"></a>
	</form>
	</td>
</tr>
</table>
