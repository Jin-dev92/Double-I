...
if($mode=="update"){
	...
	sql_query($sql);
	alert("선택한 인원에 대해서 탈퇴신청 처리되었습니다","supply_list.php?$qstr");
	exit;
}
...
$sql_search = " where is_supply='1' and mb_leavedatetime='' ";
...
if($mode=="excel"){
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue("Q1", "공급업체명")
				->setCellValue("R1", "공급업체 전화번호");
	...
	for($i=2; $row=sql_fetch_array($result); $i++){    
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue("Q$i", "$row[supply_name]")
					->setCellValue("R$i", "$row[mb_tel]");
	}
...
<tr>
	...
	<select name="sfl">
	<?
	$sfl_txt_array = array("아이디","이름","이메일","핸드폰","성별","공급업체명");
	$sfl_val_array = array("mb_id","mb_name","mb_email","mb_hp","mb_sex","supply_name");
	...
	<a href="supply_list.php?mode=excel&<?=$qstr?>"><img src="img/ex_down.gif" alt="엑셀다운" align="absmiddle"></a>
	</form>
	</td>
</tr>
</table>	

<form name="flist" id="flist" method="post" action="supply_list.php">
...
<tr onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor=''" bgcolor="#FFFFFF">
	...
	<td><a href="supply_form.php?mb_no=<?=$row[mb_no]?>&<?=$qstr?>"><img src="img/su_btn.gif"></a></td>
	<td><a href="javascript:del('supply_list.php?mode=update&mb_no=<?=$row[mb_no]?>&<?=$qstr?>')"><img src="img/del_btn.gif"></a></td>
</tr>
<?
}
...
<div class="btn_cen"><a href="javascript:nfor_list('탈퇴신청','list_update')"><img src="img/sun_del.gif" alt="선택삭제"></a><a href="supply_form.php?<?=$qstr?>"><img src="img/dong_btn.gif"></a></div>
</form>
<?
include "tail.php";
?>