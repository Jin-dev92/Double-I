...
if($mode=="list_update"){
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update nfor_keyword set wr_rank='{$_POST['wr_rank'][$k]}',
										wr_keyword='{$_POST['wr_keyword'][$k]}',
										wr_current='{$_POST['wr_current'][$k]}'
...
if($mode=="list_delete"){
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("delete from nfor_keyword where wr_id='{$_POST['wr_id'][$k]}'");
	}
...
if($mode=="delete"){
	sql_query("delete from nfor_keyword where wr_id='$wr_id'");
	alert("정상적으로 삭제되었습니다","$PHP_SELF?$qstr");
}

$sql_common = " from nfor_keyword ";
$sql_search = " where (1) ";
...
<form name="flist" id="flist" method="post" action="keyword_list.php">
...
<tr onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor=''" bgcolor="#FFFFFF">
	...
	<td><input type="text" class="input_txt" name="wr_rank[<?=$i?>]" size="4" value="<?=$row[wr_rank]?>"></td>
	<td><input type="text" class="input_txt" name="wr_keyword[<?=$i?>]" value="<?=$row[wr_keyword]?>"></td>
	<td><input type="text" class="input_txt" name="wr_current[<?=$i?>]" size="4" value="<?=$row[wr_current]?>"></td>
	<td><a href="keyword_form.php?wr_id=<?=$row[wr_id]?>&<?=$qstr?>"><img src="img/su_btn.gif"></a></td>
	<td><a href="javascript:del('keyword_list.php?mode=delete&wr_id=<?=$row[wr_id]?>&<?=$qstr?>')"><img src="img/del_btn.gif"></a></td>
</tr>
<?
}
...
<a href="keyword_form.php?<?=$qstr?>"><img src="img/dong_btn.gif"></a></div>
</form>
<?
include "tail.php";
?>