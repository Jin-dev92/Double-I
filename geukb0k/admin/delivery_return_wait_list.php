...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		...
		$chk_cnt = sql_fetch("select count(*) as cnt from nfor_cart where cart_id='$ct[cart_id]' and delivery_step='4'");
		sql_query("update nfor_cart set delivery_step='4' where ct_id='{$_POST['ct_id'][$k]}'");
		if(!$chk_cnt[cnt]){
			$order = sql_fetch("select * from nfor_order where cart_id='$ct[cart_id]'");
			nfor_send("delivery_return",$order[mb_email],$order[mb_hp],$order[mb_no],$order[od_id],$ct[ct_id]);
		}
	}

	alert("일괄 반품 처리되었습니다","delivery_return_wait_list.php?$qstr");
	exit;
}


$sql_common = " from nfor_cart ";
$sql_search = " where delivery_step='3' and it_delivery='1' ";	// pay_step='2' and
...
<tr>
	...
	<a href="delivery_return_wait_list.php?mode=excel&<?=$qstr?>"><img src="img/ex_down.gif" alt="엑셀다운" align="absmiddle"></a>
	</form>
	</td>
</tr>
</table>
...
<form name="flist" id="flist" method="post" action="delivery_return_wait_list.php">
...
<div class="btn_cen"><a href="javascript:nfor_list('반품','list_update')"><img src="img/delivery11111_btn.gif" alt="일괄반품처리"></a></div>
</form>
