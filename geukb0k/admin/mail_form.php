...
if($wr_id) $write = sql_fetch("select * from nfor_mail where wr_id='$wr_id'");
...
if($mode){
	...
	if(!$wr_subject) alert("메일제목을 입력하셔야 이용가능합니다");
	if(!$wr_memo) alert("메일내용을 입력하셔야 이용가능합니다");

	if($mode=="insert"){
		sql_query("insert nfor_mail set ca_name='$ca_name',wr_subject='$wr_subject',wr_memo='$wr_memo', wr_datetime=NOW(), send_email='$send_email', who='$who'");
		alert("정상적으로 등록되었습니다","mail_list.php?$qstr");
	}

	if($mode=="update"){
		sql_query("update nfor_mail set ca_name='$ca_name',wr_subject='$wr_subject',wr_memo='$wr_memo', send_email='$send_email', who='$who' where wr_id='$wr_id'");
		alert("정상적으로 수정되었습니다","mail_form.php?wr_id=$wr_id&$qstr");
	}
	exit;
}
...
<tr>
	<th>메일제목</th>
	<td><input type="text" class="input_txt" name="wr_subject" id="wr_subject" value="<?=$write[wr_subject]?>" style="width:100%;"></td>
</tr>
<tr>
	<th>메일내용</th>
...
<tr>
	<th>발송메일임의지정</th>
	<td><input type="text" name="send_email" id="send_email" value="<?=$write[send_email]?$write[send_email]:$config[cf_email]?>"></td>
</tr>
</table>
<div class="btn_cen"><input type="image" src="img/dong_btn.gif"> <a href="mail_list.php?<?=$qstr?>"><img src="img/list.gif"></a></div>
</form>
