...
if($mode=="list_delete"){
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		$write = sql_fetch("select * from nfor_cooperation where wr_id='{$_POST['wr_id'][$k]}'");
		@unlink($nfor[path]."/data/cooperation/".$write[wr_upload]);		
		sql_query("delete from nfor_cooperation where wr_id='{$_POST['wr_id'][$k]}'");
	}
	alert("정상적으로 삭제되었습니다","cooperation_list.php?$qstr");
	exit;
}

if($mode=="delete"){
	$write = sql_fetch("select * from nfor_cooperation where wr_id='$wr_id'");
	@unlink($nfor[path]."/data/cooperation/".$write[wr_upload]);
	sql_query("delete from nfor_cooperation where wr_id='$wr_id'");
	alert("정상적으로 삭제되었습니다","cooperation_list.php?$qstr");
	exit;
} 

if($mode=="change"){
	sql_query("update nfor_cooperation set wr_is_reply='$wr_is_reply' where wr_id='$wr_id'");
	die("change");
	exit;
}

$sql_common = " from nfor_cooperation ";
$sql_search = " where (1) ";
...
<tr>
	<td>
	<form name="search" method="get" action="cooperation_list.php">
	<select name="ca_name" id="ca_name" onchange="search.submit()">
	<option value="">문의분류
	<?
	for($i=0; $i<count($nfor[cooperation]); $i++){
	?>	
	<option value="<?=$nfor[cooperation][$i]?>" <? if($ca_name==$nfor[cooperation][$i]) echo "selected"; ?>><?=$nfor[cooperation][$i]?>
	<? } ?>
	</select>
...
<form name="flist" id="flist" method="post" action="cooperation_list.php">
...
<tr onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor=''" bgcolor="#FFFFFF">
	...
	<td style="text-align:left; padding:10px; word-break:break-all;"><span class="tex02"><?=$row[ca_name]?"[".$row[ca_name]."] ":" "?></span>&nbsp;<a href="cooperation_form.php?wr_id=<?=$row[wr_id]?>&<?=$qstr?>" class="left_menu"><?=cut_str($row[wr_subject],90)?></a></td>
	...
	if($row[wr_is_reply]=="2"){	
		if($row[wr_reply_subject]){
			echo "<a href=\"#\" onclick=\"javascript:window.open('formmail.php?wr_type=cooperation&wr_id=$row[wr_id]','메일보내기','toolbar=no, width=600, height=500, left=0, top=0');\"><img src='img/ju_btn10.jpg' alt='답변보기'></a>";		
		} else{
	...
	?></td>
	<td><a href="cooperation_form.php?wr_id=<?=$row[wr_id]?>&<?=$qstr?>"><img src="img/su_btn.gif"></a></td>
	<td><a href="cooperation_list.php?mode=delete&wr_id=<?=$row[wr_id]?>&<?=$qstr?>"><img src="img/del_btn.gif"></a></td>
</tr>
<?
}
