...
/* =   저작권			:	(주)다우기술                        = */
/* ========================================================================== */
require $nfor[pg_path]."/config/keycode.cfg";	
define( "PAYTYPE"	,"C" );
...
function CardOrderSugi( $server_ip, $port, $cpid, $enckey, $timeout ) 
{
	...
	global $res_orderno;
	global $res_amount;
	global $res_orderdate;
	global $res_daoutrx;
	global $res_kcpgid;
	global $res_cpname;
	global $res_escrowflag;
	...
	$res_orderdate		= $recvArray[4];
	$res_daoutrx		= $recvArray[5];
	$res_kcpgid			= $recvArray[6];
	$res_cpname			= $recvArray[7];
	$res_escrowflag		= $recvArray[8];
	$res_daoureserved1	= $recvArray[9];
	$res_daoureserved2	= $recvArray[10];
}
...
function CardAuthSugi( $server_ip, $port, $cpid, $enckey, $timeout ) 
{
	...
	$res_daoureserved1	= $recvArray[12];
...
/* =   저작권			:	(주)다우기술      	            = */
/* ========================================================================== */
function CardAutoKeygen( $server_ip, $port, $cpid, $enckey, $timeout ) 
{
    global $opcode;
	global $PAYMETHOD;
	global $ORDERNO;
	...
	global $res_gendate;
  	...
  	$res_resultcode  	= $recvArray[0];
  	$res_errormessage	= $recvArray[1];
  	$res_autokey      	= $recvArray[2];
  	$res_gendate       	= $recvArray[3];
}
...
/* =   저작권			:	(주)다우기술      	            = */
/* ========================================================================== */
function CardAutoBatch( $server_ip, $port, $cpid, $enckey, $timeout ) 
{
    global $opcode;
	global $REBATCHFLAG;
	global $ORDERNO;
	...
	global $res_cptelno;
  	...
  	$res_orderno      	= $recvArray[2];
  	$res_amount       	= $recvArray[3];
  	$res_daoutrx      	= $recvArray[4];
  	$res_authno		   	= $recvArray[5];
	$res_nointflag		= $recvArray[6];
	$res_quota			= $recvArray[7];
  	$res_authdate     	= $recvArray[8];
  	$res_cpname       	= $recvArray[9];
  	$res_cpurl        	= $recvArray[10];
  	$res_cptelno      	= $recvArray[11];
}
