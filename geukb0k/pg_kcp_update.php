...
include_once "path.php";
    ...
    include "$nfor[pg_path]/cfg/site_conf_inc.php";       // 환경설정 파일 include
    require "$nfor[pg_path]/pp_cli_hub_lib.php";              // library [수정불가]
	...
	$pg_price = $order[$order[payment_type]."_price"];
    ...
    if ( $req_tx == "pay" )
    {
            ...
            $c_PayPlus->mf_set_ordr_data( "ordr_mony",  $pg_price );                   
    ...
    if ( $req_tx == "pay" )
    {
        if( $res_cd == "0000" )
        {
             ...
             if ( $use_pay_method == "000000001000" )
            {
				...
				$result_url = "order_view.php?od_id=$order[od_id]";
	...
	$chk_order = sql_fetch("select * from nfor_order where od_id='$od_id' and pay_step='1'");
	if($chk_order[od_id]){
		$bSucc = "";
	} else{
...
?>
    ...
    <head>
        ...
        <script type="text/javascript">
            function goResult()
            {
                var openwin = window.open( '<?=$nfor[path]?>/pg/kcp/proc_win.html', 'proc_win', '' )
				<? if($bSucc == "false"){ ?>
				alert("<?=iconv("euc-kr","utf-8",$res_msg)." ".$res_cd?>");
				<? } ?>
    ...
    <form name="pay_info" method="post" action="<?=$result_url?>">
        ...
        <input type="hidden" name="card_name"         value="<?=iconv("euc-kr","utf-8",$card_name)      ?>">    <!-- 카드명 -->
