...
include_once $nfor[skin_path]."lib/NicepayLite2.php";
...
<style>
.payfin_area{display: none;}
.row{margin-bottom: 20px;}
.row .cartimg{width:100%; height:100px; background-color:#42aeec; margin-bottom:20px;}
.row .cartimg img{width: 100%;height: 100px;}
.cart_item_wrap { background-color:#fff; border:solid 1px #ccc; margin-bottom:10px; }

ul{padding-left: 0;}

.it_name_wrap { position:relative; padding:10px; background-color:#f9f9f9; border-bottom:solid 1px #ccc; }
...
.ea_sum_box b { display:block; background-color:#f4f4f4; border-bottom:solid 1px #e5e5e5; font-size:13px; padding:10px 10px; text-align:center; }
.ea_sum_box span { font-size:14px; line-height:35px; }
...
.dy_box { border:solid 1px #ccc; background:-webkit-gradient(linear,left top,left bottom,from(#fff),to(#ecebf0)); box-shadow:none; padding:15px 10px; }
.dy_box_title { color:#111; font-size:15px; font-weight:bold; color:#666; }
...
#payment_btn { margin:0px auto; display:block; width:250px; height:40px; line-height:40px; text-align:center; padding:0px;  font-size:16px; font-weight:bold; background-color:#d32f2f; border:solid 1px #d32f2f; color:#fff; }
.#payment_btn:hover { background-color:#FFFFFF; border:solid 1px #d32f2f; color:#d32f2f; }

#zipcode_btn  { cursor:pointer; position:absolute; left:85px; top:5px; height:26px; display:block; width:100px; text-align:center; line-height:26px; border:solid 1px #ccc; background:-webkit-gradient(linear,left top,left bottom,from(#fff),to(#ecebf0)); box-shadow:none; }
...
.money_wrap { position:relative; width:100%; }
...
.payment_type_menu li { margin:0px; padding:0px; float:left; width:100px; position:relative;  height:30px; }
...
.dl_total_prcie dt { font-size:14px; color:#666; text-align:left; width:50%; float:left; height:35px; line-height:35px; border-bottom:solid 1px #eee; }
.dl_total_prcie dd { font-size:14px; color:#666; text-align:right; width:50%; float:left; height:35px; line-height:35px; border-bottom:solid 1px #eee; }
...
<div id="member_info">
	...
	<li class="cart_item_wrap">
		...
		<div class="ea_sum_box">
			...
			<div class="ea_sum_box1">
				<b>상품합계</b>
				<span class="it_total_price" style="color:#cc0000;font-weight:bold;"><?=number_format($ct_sum[$cart[it_id]])?>원</span>
			</div>
	...
	<div class="dy_box_wrap">
		...
		<div class="dy_bg">
			...
			<div style="border-top:solid 1px #eee; padding-top:5px; margin-top:7px;">
				...
				<div class="money_wrap" style="width:98%; border-right:1px solid #eaeaec;line-height:45px;">
	...
	<div class="dy_box_wrap">
		...
		<div class="dy_bg">
			...
			<dl class="dl_total_prcie">
				...
				<dt>총할인금액</dt>
				<dd><span id="span_discount_price" style="color:#0099ff; font-weight:bold;">0</span>원</dd>

				<dt>총결제금액</dt>
				<dd><span id="span_pay_total_price" style="color:#cc0000;font-weight:bold;"><?=number_format($total_price)?></span>원</dd>
			</dl>
	...
	<style>
	.dl_total_dl { border-top:solid 1px #eee; overflow:hidden; width:100%;   }
	.dl_total_dl td { font-size:14px; color:#666; text-align:left; height:35px; line-height:35px; border-bottom:solid 1px #eee; }
	</style>
	...
	<div class="dy_box_wrap">
		...
		<div class="dy_bg">
			...
			<table class="dl_total_dl">
			<tr>
				<td width="15%"><label>이름</label></td>
				<td><input type="text" name="dy_name" id="dy_name" placeholder="받으시는분 이름" value="<?=$member[mb_name]?>"  style="width:185px; border:solid 1px #DCDCDC;height:24px " ></td>
			</tr>
			<tr>
				<td><label>휴대전화</label></td>
				<td><input type="number" pattern="[0-9]*" name="dy_hp" id="dy_hp" value="<?=str_number($member[mb_hp])?>" placeholder="받으시는분 휴대전화번호" style="width:185px; border:solid 1px #DCDCDC;height:24px " ></td>
			</tr>
			<tr>
				<td rowspan="2"><label>주소</label></td>
				<td>
				<div style=" height:40px; position:relative;">
				<input type="number" pattern="[0-9]*" name="dy_zip" id="dy_zip" placeholder="우편번호" value="12345" readonly  style="width:85px; border:solid 1px #DCDCDC;height:24px "><a id="zipcode_btn">우편번호찾기</a>
				</div>
			</td>
			</tr>
			<tr>
			<td >
			<input type="text" name="dy_addr1" id="dy_addr1" placeholder="주소" value="<?=$member[mb_addr1]?>" style="margin:5px 0px; width:185px; border:solid 1px #DCDCDC;height:24px;" readonly  >
			<input type="text" name="dy_addr2" id="dy_addr2" placeholder="상세주소" value="<?=$member[mb_addr2]?>" style="width:185px; border:solid 1px #DCDCDC;height:24px " >
			</td>
...
<!-- 결제하기 -->
<div class="dy_box_wrap" style=" margin:20px 0px;">
	...
	<div class="dy_bg">
		...
		<div id="payment_iche" class="payment_wrap" ></div>
</form>
		...
		*/
		...
		// $query1=sql_query("SELECT * FROM nfor_cart WHERE mb_no=".$member[mb_no].";");
		// $cart_row=sql_fetch_array($query1);
		// $query2=sql_query("SELECT * FROM nfor_order WHERE cart_id=".$cart_row[cart_id].";");
		...
		$nicepay->m_MID         = "dublechi1m";
		 // 상점아이디
		$nicepay->m_Moid        = $order_id;
		// 상품주문번호
		$nicepay->m_Price       = $total_price;
		// 결제상품금액
		$nicepay->m_BuyerEmail  = $member[mb_email];
		// 구매자메일주소
		$nicepay->m_BuyerName   = $member[mb_id];
		// 구매자명
		$nicepay->m_BuyerTel    = $member[mb_hp];
		// 구매자연락처
		$nicepay->m_GoodsCode   = $g_code;
		// 결제상품코드
		$nicepay->m_GoodsName   = $item[it_name]."(".$opt_name.")";
		// 결제상품명
		$goodsCnt               = "1";
		// 결제상품개수
		...
		*/
		...
		?>
		<script src="https://web.nicepay.co.kr/flex/js/nicepay_tr_utf.js" type="text/javascript"></script>
		<form name="payForm" method="post" action="/skin/demo/payResult_utf.php" class="payForm">
							...
							<div class="tabletypea">
								<table>
									...
									<tr>
										...
										<td><input type="text" name="GoodsCnt" value="<?php $goodsCnt?>" class="GoodsCnt"></td>
									</tr>
									<tr>
...
<div style="margin:0px auto; width:550px;" id="display_pay_button" style="display:block">
...
<script type="text/javascript">
	// function countPN() {
	var a=document.getElementsByClassName('option_count');
	var b="";
	var idx=0;
	for (var i = 0; i < a.length; i++) {
		b=document.getElementsByClassName('option_count')[i].innerText;
		// console.log(b);
		var cnt=b.split(" ")[1].split("개")[0];
		idx+=parseInt(cnt);
	}
	// console.log(idx);
	$('.GoodsCnt').val(idx);
