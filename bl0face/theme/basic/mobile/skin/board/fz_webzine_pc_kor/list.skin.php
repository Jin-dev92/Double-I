...
<div id="bo_list" class="fz_wrap">
			...
			<a href='<?php echo $list[$i]['href'];?>' class="fz_gallery_li_wrap">
				...
				<span class="fz_gallery_box">
					...
					<?php 
						if($bo_table == "event_kor") {
					?>
					<span class="fz_gallery_date_2">이벤트기간 : <?php echo $list[$i]['wr_1'] ?>  ~  <?php echo $list[$i]['wr_2'] ?></span>
					<?php
						}
						?>
	...
	<div class="fz_footer">
        ...
        <?php } ?>
		<div class="fr">
            <?php if ($list_href) { ?><input type="button" class="list_btn btn_list" value="목록" alt="목록" onclick="location.href='<?php echo $list_href ?>'"/><?php } ?>
            ...
            <?php if ($write_href) { ?><input type="button" class="list_btn btn_write" value="글쓰기" alt="글쓰기" onclick="location.href='<?=$write_href?>'" /><?php } ?>
		</div>
	</div>
    </form>
...
<fieldset id="bo_sch">
	...
	<input type="submit" class="btn_search_submit" value="검색" alt="검색" />
    </form>
</fieldset>
<!-- } 게시판 검색 끝 -->
</div>
