...
include_once(G5_PLUGIN_PATH.'/jquery-ui/datepicker.php'); //달력 플러그인
...
<section id="bo_w" class="fz_wrap">
	...
	?>
			FILLER REVIEW
	<?php
	} else if($bo_table == "filler_lab_kor"){
	?>
			FILLER LAB
	<?php
	}
	...
	<div class="bo_w_form">
		<div class="bo_w_title<?php if($is_category){ echo " use_category";}?>">
			...
			?>
		<div class="bo_w_date">
			<ul style="margin:0; padding:0; overflow:auto; list-style-type:none;">
				<li style="float:left; display:inline; margin-right:10px;"><input type="text" name="wr_1" value="<?php echo $write["wr_1"] ?>" id="wr_1" required readonly class="i_date_large required" size="10" maxlength="25" placeholder="이벤트시작날짜"></li>
				<li style="float:left; display:inline; margin-right:10px; width: 10px; height: 31px; line-height: 30px;"><p style="font-size:14px;">~</p></li>
				<li style="float:left; display:inline"><input type="text" name="wr_2" value="<?php echo $write["wr_2"] ?>" id="wr_2" required readonly class="i_date_large required" size="10" maxlength="25" placeholder="이벤트종료날짜"></li>
			</ul>
		</div>
		<?php
			}
	...
	<div class="write_foot">
		<input type="submit" id="btn_submit" class="btn_submit" value="글쓰기" alt="글쓰기" />
		<input type="button" class="list_btn btn_list" value="목록" alt="목록" onclick="location.href='./board.php?bo_table=<?=$bo_table?>'"/>
	</div>
    ...
    $(function(){
        $("#wr_1").datepicker({ changeMonth: true, changeYear: true, dateFormat: "yy-mm-dd", showButtonPanel: true, yearRange: "c-99:c+99", minDate: "+d;", maxDate: "+365d;" });
