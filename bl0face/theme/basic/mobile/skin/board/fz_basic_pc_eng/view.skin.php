...
<h2 id="container_title">
    ...
    <div id="bo_v_top">
        ...
        <ul class="bo_v_nb">
            <?php if ($copy_href) { ?><li><a href="<?php echo $copy_href ?>" class="list_btn btn_copy" onclick="board_move(this.href); return false;">Copy</a></li><?php } ?>
            <?php if ($move_href) { ?><li><a href="<?php echo $move_href ?>" class="list_btn btn_move" onclick="board_move(this.href); return false;">Move</a></li><?php } ?>
        </ul>
        <?php } ?>

        <ul class="bo_v_com">
            <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>" class="list_btn btn_edit"><i class="fa fa-rotate-right"></i>Edit</a></li><?php } ?>
            <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" class="list_btn btn_del" onclick="del(this.href); return false;"><i class="fa fa-remove"></i>Delete</a></li><?php } ?>
            <li><a href="<?php echo $list_href ?>" class="list_btn btn_list"><i class="fa fa-list"></i>List</a></li>
            <?php if ($reply_href) { ?><li><a href="<?php echo $reply_href ?>" class="list_btn btn_reply">Reply</a></li><?php } ?>
            <?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="list_btn btn_write"><i class="fa fa-pencil"></i>Write</a></li><?php } ?>
        </ul>
        <?php
	...
	<section id="bo_v_info">
		...
		<div class="fr">
			<span class="bo_v_date"><i class="fa fa-calendar-check-o"><span class="sound_only">Date Created</span></i> <strong><?php echo date("y-m-d H:i", strtotime($view['wr_datetime'])) ?></strong></span> <span class="bar">|</span>
			<span class="bo_v_hit"><i class="fa fa-eye"></i>Count<strong><?php echo number_format($view['wr_hit']) ?></strong></span> <span class="bar">|</span>
			<span class="bo_v_comment"><i class="fa fa-comments"></i>Reply<strong><?php echo number_format($view['wr_comment']) ?></strong></span>
		</div>
	</section>
	...
	<?php
		...
		foreach($view[file] as $value) {
			if(is_array($value) && $value[source] && !$value[view]) {
	...
	<div class="fz_middle_btn">
		<div class="fl">
			<?php if ($prev_href) { ?><a href="<?php echo $prev_href ?>" class="list_btn btn_prev">Prev</a><?php } ?><?php if ($next_href) { ?><a href="<?php echo $next_href ?>" class="list_btn btn_next">Next</a><?php } ?>
		</div>
		<div class="fr">
			<?php
				include_once(G5_SNS_PATH."/view.sns.skin.php");
				if ($scrap_href) { echo "<a href='{$scrap_href}' target='_blank' class='list_btn btn_scrap' onclick='win_scrap(this.href); return false;'>Scrap</a>";}
			?>
		</div>
	</div>
<?php 
		...
		?>
		<?PHP
			} else if($bo_table == "online_kor" || $bo_table == "online_eng" || $bo_table == "online_chn") {
			 // 코멘트 입출력
