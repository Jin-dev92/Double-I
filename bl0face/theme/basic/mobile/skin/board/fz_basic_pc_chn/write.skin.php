...
$ad_btn = "";
if($is_file) {
	$ad_btn .= "<a id='ad-btn_file' href=\"javascript:ad_layer('file');\" class='btn_file' title='附加文件'><span>附加文件</span>";
	if($w=="u") $ad_btn .= "<em>{$file['count']}</em>";
	$ad_btn .= "</a>";
}

if($is_link) {
	$ad_btn .= "<a id='ad-btn_link' href=\"javascript:ad_layer('link');\" class='btn_link' title='链接'><span>链接</span>";
...
<div id="sub_title">
		<ul class="sub_ul">
			...
			<span>-</span>
			<li class="sub_li02"><img src="/theme/basic/include/img/sub_filler_icon.png">BIOFACE STORY</li>
		</ul>
</div>
...
<section id="bo_w" class="fz_wrap">
	...
	<div class="bo_w_form">
		<div class="bo_w_title<?php if($is_category){ echo " use_category";}?>">
			...
			<span class="placeholder">
				...
				<label for="wr_subject">填写标题</label>
			</span>
		</div>
		<?php if($option){?>
		...
		<div class="bo_write_mbinfo">
			<ul>
				<?php if($is_name) { ?>
				<li><span class="placeholder"><input type="text" name="wr_name" value="<?php echo $name; ?>" id="wr_name" required class="i_text required" maxLength="20"><label for='wr_name'>名字</label></span></li>
				<?php }?>
				<?php if($is_password){?>
				<li><span class='placeholder'><input type="password" name="wr_password" id="wr_password" style="width:200px;"<?php echo $password_required ?> class="i_text <?php echo $password_required ?>" maxLength="20"><label for="wr_password" style="width:200px;">密码为手机号</label></span></li>
				<?php }?>
				<?php if($is_email){?>
				<li><span class="placeholder"><input type="text" name="wr_email" value="<?php echo $email ?>" id="wr_email" class="i_text email" maxlength="100"><label for="wr_email">邮箱</label></span></li>
				<?php }?>
			</ul>
		...
		<div class="bo_editor_wrap">
			...
			<?php 
				...
				if(!$is_dhtml_editor){echo "<label for='wr_content'>请填写内容.</label></span>";}
			?>
			<?php if($write_min || $write_max) { ?>
			<!-- 최소/최대 글자 수 사용 시 -->
	...
	<div class="write_foot">
		...
		-->
		<input type="submit"id="btn_submit" class="list_btn btn_write" value="发表" alt="发表" />
		<input type="button" class="list_btn btn_write" value="目录" alt="目录" onclick="location.href='./board.php?bo_table=<?=$bo_table?>' "/>
