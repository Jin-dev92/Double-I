...
if(!!(FALSE !== strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'mobile')) != 1){
  ...
  if(isset($_GET['approach'])){
  	header("Location: /Lifting/rijuran?approach=keyword");
  } else {
    header("Location: /Lifting/rijuran");
  }
} else {
	//MOBILE
}
...
<div class="sub_img">
	<img src="../assets/img/sub/Lifting/rijuran_m.jpg?<?=rand()?>">
</div>
...
if(approach){
  $('.sub_img img').attr('src','../assets/img/sub/Lifting/rijuran_db_m.jpg?<?=rand()?>');
}

	$('.m_header_info li:eq(1)').trigger('click');
	$('.m_header_tab ul:eq(1) li:eq(4)').addClass('active');
});
</script>
...
if($('.agree').is(":checked") == false) {
  ...
  firebase.database().ref('rejuran/users/').once('value').then(function(snapshot){
    ...
    firebase.database().ref('rejuran/users/' + userKey).set({
      name : name,
      phone : phone1,
      contents: content,
      can_date : '',
