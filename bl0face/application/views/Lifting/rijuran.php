...
if(!!(FALSE !== strstr(strtolower($_SERVER['HTTP_USER_AGENT']), 'mobile')) != 1){
  ...
  if(isset($_GET['approach'])){
    header("Location: /Lifting/M_rijuran?approach=keyword");
  } else {
    header("Location: /Lifting/M_rijuran");
  }
}
...
<div class="sub_img">
	<img src="../assets/img/sub/Lifting/rijuran.jpg?<?=rand()?>">
</div>
...
if(approach){
  $('.sub_img img').attr('src','../assets/img/sub/Lifting/rijuran_db.jpg?<?=rand()?>');
}
...
if($('.agree').is(":checked") == false) {
  ...
  firebase.database().ref('rijuran/users/').once('value').then(function(snapshot){
    ...
    firebase.database().ref('rijuran/users/' + userKey).set({
      name : name,
      phone : phone1,
      contents: content,
      can_date : '',
