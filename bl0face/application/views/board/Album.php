    ...
    <ul class="album_ul">
      ...
      $page_min = $page_max- $list['page_limit'];
        foreach ($list as $entry) {
          ...
          ?>
          <li><a href="/board/view/<?=$this->uri->segment(3)?>/<?=$entry['wr_id']?>">
            <div class="album_info_wrap">

            <div class="album_title"><?=$entry['wr_subject']?></div>
            <div class="album_name">
              <?
              if($entry['mb_id'] == '' || !$entry['mb_id']) { echo $entry['wr_name'];
              }else{
                if($entry['mb_id'] == 'admin'){
                  echo '비오페이스';
                } else {
                  echo $entry['mb_id'];
                } 
            ...
            <div class="album_img">
              ...
              for ($i=0; $i < $cnt_img; $i++) {
                if ($list['album_img'][$i]['wr_id']==$entry['wr_id']) {
                  ...
                  ?>
                    <img src="/data/file/<?=$list['album_img'][$i]['bo_table']?>/<?=$list['album_img'][$i]['bf_file']?>" alt="">
