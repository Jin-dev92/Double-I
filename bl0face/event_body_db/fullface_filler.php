...
<div class="row">
	<h1 style="text-align: center;">Full Face Filler 페이지</h1>
...
$(document).ready(function(){
	...
	firebase.database().ref('fullface_filler/users/').on('value', function(snapshot){
		$('.contents').empty();
		...
		if(pick == ''){
			...
			for(var i = 0; i < arr_size ; i++){
				...
				firebase.database().ref("fullface_filler/users/").on("value", function(snapshot2) {
				    ...
				    var wishtime = new_arr[snapshotToArray(snapshot)[i]].can_date;
					...
					idx_arr[i] = {idx:idx, contents:contents, name:name, wishtime:wishtime , phone:phone , process_state:process_state , memo:memo , origin:origin , reg_time:reg_time};
			...
			for(var i = 0; i < arr_size ; i++){
				firebase.database().ref("fullface_filler/users/" + re_reverse[i]).on("value", function(snapshot2) {
					


				    var name = snapshot2.child("name").val();
				    var wishtime = snapshot2.child("can_date").val();
				    ...
				    all_arr += [name, wishtime, phone, contents, process_state, memo, origin, reg_time, idx];



				    if(paging-idx >= <?=$startRow?> && paging-idx < <?=$startRow+$pageSize?>){
				    	
				    	var fff = '<?=$startRow+$pageSize?>';
				    	var idx_code = snapshotToArray(snapshot)[ddd];
				    	console.log(idx_code);
					    var push = '<tr><td>' + idx + '</td><td>' + name + '</td><td>' + reg_time + '</td><td>' + wishtime + '</td><td>' + phone + '</td><td>' + contents + '</td><td>' + origin + '</td><td><input type="hidden" value="' + ddd + '"><a href="#modal1" class="modal-trigger" onclick="addindex(this)">' + process_state + '</a></td><td><input type="hidden" value="' + ddd + '"><input type="text" value="' + memo + '" onkeydown=changeMemo(this)></td></tr>';
					}
			...
			for(var i = 0; i < arr_size ; i++){
				firebase.database().ref("fullface_filler/users/" + re_reverse[i]).on("value", function(snapshot2) {
				    ...
				    if(cprt == pick){
						    ...
						    var push = '<tr><td>' + idx + '</td><td>' + name + '</td><td>' + reg_time + '</td><td>' + wishtime + '</td><td>' + phone + '</td><td>' + contents + '</td><td>' + origin + '</td><td><input type="hidden" value="' + snapshotToArray(snapshot)[i] + '"><a href="#modal1" class="modal-trigger" onclick="addindex(this)">' + process_state + '</a></td><td><input type="hidden" value="' + snapshotToArray(snapshot)[i] + '"><input type="text" value="' + memo + '" onkeydown="changeMemo(this)"></td></tr>';
						    ...
						    firebase.database().ref("fullface_filler/users/").on("value", function(snapshot2) {
								...
								idx_arr[xx] = {idx:idx, contents:contents, name:name, wishtime:wishtime , phone:phone , process_state:process_state , memo:memo , origin:origin , reg_time:reg_time};
...
function changeprocess(value){
	...
	firebase.database().ref('fullface_filler/users/' + index).update({
		process : value,
	});
