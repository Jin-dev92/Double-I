...
<div class="row">
	<h1 style="text-align: center;">SHURINK 페이지</h1>
...
$(document).ready(function(){
	...
	firebase.database().ref('shurink/users/').on('value', function(snapshot){
		$('.contents').empty();
		...
		if(pick == ''){
			...
			for(var i = 0; i < arr_size ; i++){
				...
				firebase.database().ref("shurink/users/").on("value", function(snapshot2) {
			...
			for(var i = 0; i < arr_size ; i++){
				firebase.database().ref("shurink/users/" + re_reverse[i]).on("value", function(snapshot2) {
...
function changeprocess(value){
	...
	firebase.database().ref('shurink/users/' + index).update({
		process : value,
	});
