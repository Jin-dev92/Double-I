...
<div class="row">
	<h1 style="text-align: center;">RIJURAN 페이지</h1>
...
$(document).ready(function(){
	...
	firebase.database().ref('rijuran/users/').on('value', function(snapshot){
		$('.contents').empty();
		...
		if(pick == ''){
			...
			for(var i = 0; i < arr_size ; i++){
				...
				firebase.database().ref("rijuran/users/").on("value", function(snapshot2) {
			...
			for(var i = 0; i < arr_size ; i++){
				firebase.database().ref("rijuran/users/" + re_reverse[i]).on("value", function(snapshot2) {
...
function changeprocess(value){
	...
	firebase.database().ref('rijuran/users/' + index).update({
		process : value,
	});
